...
const contract = (vos: number[]) => ({
  id: 'c1', code: 'RU-CON-0188', valueUSD: 1_000_000, termDays: 540,
...
describe('variation-order cap (18.1 — ≤ 10% of contract value)', () => {
  it('refuses a VO that pushes the total past 10% and audits the refusal', async () => {
    ...
    await expect(svc.addVariationOrder(ROC, 'c1', { valueUSD: 10_000, approvedOn: '2026-06-01' })).rejects.toBeInstanceOf(BadRequestException);
    expect(tx.variationOrder.create).not.toHaveBeenCalled();
    expect(audit.record).toHaveBeenCalledWith('u-roc', 'VO_ADD_REFUSED', 'RU-CON-0188 (18.1)');
  });

  it('accepts a VO within the cap', async () => {
    const { svc, tx, audit } = makeService(contract([50_000])); // + 10k → 6%
    await svc.addVariationOrder(ROC, 'c1', { valueUSD: 10_000, approvedOn: '2026-06-01' });
    expect(tx.variationOrder.create).toHaveBeenCalledOnce();
    expect(audit.record).toHaveBeenCalledWith('u-roc', 'VO_ADD', 'RU-CON-0188');
  });
});
