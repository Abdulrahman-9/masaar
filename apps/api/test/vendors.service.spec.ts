...
const ROC: AuthUser = { userId: 'u-roc', name: 'ROC', role: 'ROC_ADMIN' };
...
describe('vendor ban cap (14.3 — ≤ 12 months)', () => {
  it('refuses a ban beyond 12 months and audits the refusal', async () => {
    ...
    await expect(svc.ban(ROC, 'v1', { banUntil: isoInMonths(18), reason: 'رفض توقيع عقد محال بلا عذر مقبول' })).rejects.toBeInstanceOf(BadRequestException);
  ...
  it('applies a ban within 12 months (transaction + audit)', async () => {
    const { svc, prisma, audit } = makeService(base);
    await svc.ban(ROC, 'v1', { banUntil: isoInMonths(6), reason: 'رفض توقيع عقد محال بلا عذر مقبول' });
...
describe('vendor suspend guard', () => {
  it('refuses to suspend an already-suspended vendor', async () => {
    ...
    await expect(svc.suspend(ROC, 'v1', 'سبب موثّق كافٍ الطول لتجاوز عشرين حرفًا')).rejects.toBeInstanceOf(BadRequestException);
    expect(prisma.$transaction).not.toHaveBeenCalled();
  });

  it('suspends an active vendor (transaction + event + audit)', async () => {
    const { svc, prisma, audit } = makeService(base);
    await svc.suspend(ROC, 'v1', 'سبب موثّق كافٍ الطول لتجاوز عشرين حرفًا');
    expect(prisma.$transaction).toHaveBeenCalledOnce();
