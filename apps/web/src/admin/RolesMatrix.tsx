...
import {
  cellState, capsForRole, holdersOf, holdersOfRole, impactfulCount, readCount, domainsFor,
  roleKey, roleTone, specConflicts, type CellState,
} from './access';
import {
...
export default function RolesMatrix() {
  ...
  const [tab, setTab] = useState<'cap' | 'role'>('cap');
  ...
  return (
    <div className="op-page" style={{ maxWidth: 1240 }}>
      <div className="op-page__head">
        <div>
          <h1 className="op-page__title">{t('caps.title')}</h1>
          <div className="op-page__sub">{t('caps.sub', { n: fmtCount(CAPABILITIES.length, lang) })}</div>
        </div>
      ...
      <div className="file-tabs">
        <button className={`file-tab${tab === 'cap' ? ' file-tab--on' : ''}`} onClick={() => setTab('cap')}>{t('caps.tabByCap')}</button>
        <button className={`file-tab${tab === 'role' ? ' file-tab--on' : ''}`} onClick={() => setTab('role')}>{t('caps.tabByRole')}</button>
      </div>
      ...
      {tab === 'cap' ? (
            ...
            <div className="acc-matrix">
              <table className="op-tbl">
                ...
                <tbody>
                  ...
                  {/* excluded from every counter — shown so the 43 reconciles on screen */}
        ...
        <div className="acc-rolecards">
          {ROLES.map((r) => {
            const holders = holdersOfRole(r, state.users);
            const caps = capsForRole(r);
            return (
              <div key={r} className="ad-panel">
                <div className="ad-panel__head">
                  <div>
                    <div className="ad-panel__t">
                      <span className={`acc-role acc-role--${roleTone(r)}`}>{t(`roles.names.${roleKey(r)}`)}</span>
                    </div>
                    <div className="ad-panel__s">{r}</div>
                  </div>
                  <span className="ad-panel__count ad-panel__count--amber" style={{ marginInlineStart: 'auto' }}>
                    {fmtCount(holders.length, lang)}
                  </span>
                </div>
                <div style={{ padding: '12px 20px 16px' }}>
                  <div className="acc-rolecard__layer">{t(`caps.layer_${r}`)}</div>
                  <div style={{ display: 'flex', gap: 14, marginBlockStart: 10, fontSize: 12 }}>
                    <span><span className="acc-mono">{fmtCount(impactfulCount(r), lang)}</span> {t('caps.statActions')}</span>
                    <span><span className="acc-mono">{fmtCount(readCount(r), lang)}</span> {t('caps.statReads')}</span>
                    <span><span className="acc-mono">{fmtCount(domainsFor(r).length, lang)}</span> {t('caps.statDomains')}</span>
                  </div>
                  {holders.length === 0 ? (
                    <div className="ad-empty-inline" style={{ marginBlockStart: 10 }}>{t('caps.noHolder')}</div>
                  ) : (
                    <div className="acc-rolecard__holders">
                      {holders.map((h) => (
                  ...
                  <div style={{ marginBlockStart: 12 }}>
                    <a className="acc-open" href="#/admin/users">
                      {t('caps.openRegistry')}
                      <Icon name="chevronEnd" size={12} strokeWidth={2} className="op-chev-fwd" />
                    </a>
                  </div>
                  <div className="ad-panel__s" style={{ marginBlockStart: 8 }}>
                    {caps.length > 0 ? `${fmtCount(caps.length, lang)} · ${t('access.kpiCaps')}` : t('caps.noHolder')}
                  </div>
                </div>
              </div>
            );
