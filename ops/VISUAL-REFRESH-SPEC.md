...
--ink-4: #666F7E;       /* كان #98A1B0 → الآن 4.11…5.07 على كل الأسطح */
--ink-disabled: #98A1B0; /* جديد — قيمة ink-4 القديمة، لعناصر التحكّم المعطّلة فقط
                            (مستثناة صراحةً بـWCAG 1.4.3 «مكوّنات غير نشطة») */
...
| `--ink-4` | `#98A1B0` | **`#666F7E`** | 4.82 ✅ | 5.07 ✅ | 4.50 ✅ | 4.11 ✅ |

`--ink-4` لم يعد «عنصراً نائباً» بل «بيانات هادئة مقروءة»؛ العنصر النائب الحقيقي والمعطّل ينتقلان إلى `--ink-disabled`.
**مواضع الاستبدال الإلزامية إلى `--ink-disabled`:** `.wz-next:disabled` · `.wz-prev:disabled` · `.wz-cond__m` (العلامة غير المحقَّقة) · `.wz-check__m` · `.acc-cell--no` / `--session` · `.ctr-stage__dot--todo` · `.file-tl__later`.
...
--status-risk:        #945D17;  /* كان #B5751F → 4.79 (كان 3.32 — الأسوأ) */
...
| `risk` | `#B5751F` | **`#945D17`** | 4.79 ✅ | 5.47 | 5.20 | 4.44 |
...
--border-control: rgba(11, 19, 32, 0.46); /* جديد — 3.12:1 فوق البطاقة: حدّ كل عنصر تحكّم */
```
...
|---|---|---|
| تعديل | 43–44 | `--ink-3: #5A6474` · `--ink-4: #666F7E` |
| إضافة | بعد 44 | `--ink-disabled: #98A1B0` |
| تعديل | 57، 59، 61 | `--status-progress: #1B65A4` · `--status-done: #1C7046` · `--status-risk: #945D17` |
